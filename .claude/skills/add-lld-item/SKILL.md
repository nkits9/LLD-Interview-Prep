...
2. **Design first, cross-check second (mandatory).** Derive the design from first principles: the requirements, canonical data structures/algorithms, and this repo's principles (SOLID, hygiene rules, the concurrency toolkit in LLD.md §3). Where a known optimal DS/algorithm exists, it IS the expected answer — use it and state its complexity in the README (examples: last-move-only win check for board games; HashMap + doubly-linked list for O(1) LRU; TreeMap/heaps with price-time priority for order books; min-cash-flow graph settlement for Splitwise; lazy refill + CAS for token buckets; seat×segment inventory for train booking). THEN web-search the references — `ashishps1/awesome-low-level-design` (`problems/<name>.md`, `solutions/java/src/<name>`), Keerti Purswani's LLD repos ([github.com/keertipurswani](https://github.com/keertipurswani?tab=repositories): LLD-Design-Patterns, Uber-Ola, Swiggy-Zomato, Zerodha, Elevator, Chess, IRCTC, Meeting-Scheduler — C++, the decomposition transfers), Hello Interview / AlgoMaster write-ups, refactoring.guru — **strictly as a completeness check, never as ground truth**: these sources are unofficial and can be wrong or suboptimal. Use them only to catch requirements, entities, or edge cases the design missed — not to copy their approach. If a reference's approach differs from yours, judge it on merit (correctness, complexity, extensibility, concurrency): adopt it only if it is demonstrably better, and either way record the comparison in the README's "Approach vs alternatives" line and follow-up Q&A.
